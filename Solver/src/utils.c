    ...
    while ((c = getopt(argc, argv, "o:h:n:d:s:e:t:v:i:c:p:f:z:")) != -1) {
        switch(c) {
            ...
            default:
                ...
                fprintf(stderr, "\nExample usage:\n"CYAN"\tmpirun -n 4 ./bin/main -o \"../Data/Tmp\" -n 64 -n 64 -n 64 -s 0.0 -e 1.0 -h 0.0001 -v 1.0 -i \"TG_VORT\" -t \"TEMP_RUN\" \n"RESET);
...
void PrintSpaceVariables(const long int* N) {
    ...
    if ( !(sys_vars->rank) ) {
        for (int i = 0; i < Nx; ++i) {
            if (i < Nz_Fourier) {
                printf("x[%d]: %5.16lf\ty[%d]: %5.16lf\tz[%d]: %5.16lf\tkx[%d]: %d\tky[%d]: %d\tkz[%d]: %d\n", i, x0[i], i, run_data->x[1][i], i, run_data->x[2][i], i, k0[i], i, run_data->k[1][i],  i, run_data->k[2][i]);
            }
            else {
                printf("x[%d]: %5.16lf\ty[%d]: %5.16lf\tz[%d]: %5.16lf\tkx[%d]: %d\tky[%d]: %d\n", i, x0[i], i, run_data->x[1][i], i, run_data->x[2][i], i, k0[i],  i, run_data->k[1][i]);
            }
        }
        printf("\n\n");
    }
